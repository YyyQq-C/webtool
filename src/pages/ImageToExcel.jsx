...
function ImageToExcel() {
  ...
  const convertToExcel = async () => {
    ...
    try {
      ...
      setProgress('正在识别图片中的表格...')
      
      const res = await fetch('/bg-api/api/image-to-excel', {
        method: 'POST',
        body: formData,
      })
      
      if (!res.ok) {
        throw new Error('识别失败，请检查图片是否包含表格')
      }
      ...
      link.href = url
      link.download = `识别结果_${Date.now()}.xlsx`
      link.click()
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-[#F8FAFC] mb-2">图片转Excel</h1>
        <p className="text-[#94A3B8]">识别图片中的表格内容，生成可编辑的Excel文件</p>
      </div>
            ...
            {images.length > 0 && (
              <div className="mt-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button onClick={clearAll} className="text-sm text-[#EF4444] hover:text-[#FCA5A5]">
                    清空全部
                  </button>
                </div>
            ...
            <div className="bg-[#1E293B]/60 backdrop-blur-sm rounded-2xl p-6 border border-[#475569]">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="text-sm text-[#94A3B8]">开源OCR，基础识别</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-sm text-[#94A3B8]">AI识别，准确度高</div>
                </button>
              </div>
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          {resultUrl ? (
            <div className="bg-[#1E293B]/60 backdrop-blur-sm rounded-2xl p-6 border border-[#475569]">
              ...
              <div className="text-center py-8">
                ...
                <p className="text-[#F8FAFC] mb-4">Excel文件已生成</p>
                <button
                  onClick={downloadResult}
                  className="px-6 py-3 bg-[#3B82F6] text-white rounded-xl hover:bg-[#2563EB]"
                >
                  再次下载Excel
                </button>
              </div>
          ...
          <div className="bg-[#1E293B]/60 backdrop-blur-sm rounded-2xl p-6 border border-[#475569]">
            ...
            <div className="space-y-2 text-sm text-[#94A3B8]">
              <p>1. 上传包含表格的图片（截图、照片等）</p>
              <p>2. 图片中的表格需清晰可见</p>
              <p>3. 支持批量处理多张图片</p>
              <p>4. 识别后自动生成Excel文件</p>
            </div>
            
            <div className="mt-4 p-3 bg-[#475569]/20 rounded-lg">
              <p className="text-xs text-[#F59E0B]">⚠️ 此功能需要服务器端处理，识别准确度取决于图片清晰度</p>
            </div>
          </div>
        </div>
      </div>
